...
def main() -> int:
    ...
    # Passo 3: Empacotamento PyInstaller
    print_step("3. Gerando executável standalone (.exe) com PyInstaller")
    ...
    try:
        ...
        print_error(f"Falha ao gerar o executável com PyInstaller (código {e.returncode}).")
        return e.returncode

    if not os.path.exists(exe_path):
        print_error(f"O executável esperado não foi encontrado em: {exe_path}")
        return 1

    tamanho_mb = os.path.getsize(exe_path) / (1024 * 1024)
    elapsed = time.time() - start_time

    print_step("4. Build Concluído com Sucesso!")
    print_success(f"Executável gerado: {exe_path}")
    print(f"  • Tamanho final  : {tamanho_mb:.2f} MB")
    print(f"  • Tempo de build : {elapsed:.1f} segundos")
    print("\nVocê já pode distribuir ou executar o arquivo dist/MapaCotacoes.exe diretamente.")
    return 0
